...
import {
  AccountCircleFill,
  ListUnordered, FileList3Line, WalletLine, PaletteLine, Notification2Line, FileTextLine, QuestionLine, LogoutBoxRLine, GlobalLine, InformationLine, ArrowDropRightLine,
...
const RightPanelCategories = () => {
  ...
  return (
    <div className="w-full h-screen overflow-hidden flex flex-col dark:bg-[#121212]">
      {/* Fixed App Bar */}
      <div className="flex-shrink-0 bg-neutral-base dark:bg-[#1e1e1e] border-b border-gray-100 dark:border-gray-700">
          <div className="px-3 sm:px-4 md:px-6 lg:px-9 py-3 sm:py-4">
          <h1 className="text-lg sm:text-xl font-semibold text-[#242527] dark:text-white">{t('account_title')}</h1>
        </div>
      </div>

      {/* Scrollable Content */}
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 md:px-6 lg:px-9 py-3 sm:py-4 dark:bg-[#121212]">
        {/* Profile Header */}
        ...
        {/* Menu Section - Role-based Dynamic Items */}
          ...
          <div className="bg-neutral-base dark:bg-[#1e1e1e] rounded-2xl overflow-hidden border border-[#e6e6e6] dark:border-gray-700">
            ...
            {visiblePages.map((page, index) => {
              ...
              const handlePageClick = () => {
                if (page.id === 'update-profile') {
                  setEditingSection('profile-editor');
                } else if (page.id === 'pause-subscription') {
                  setShowPauseModal(true);
                } else if (page.id === 'cancel-subscription') {
                  setShowCancelModal(true);
                } else {
                  router.push(`/${role}/account/${page.path}`);
                }
              };

              return (
                <div key={page.id}>
                  <button className="w-full" onClick={handlePageClick}>
                    <MenuItem
                      icon={IconComponent}
                      label={t(page.label)}
                      textColor={page.id === 'cancel-subscription' ? 'text-[#bf3709]' : undefined}
                    />
                  </button>
