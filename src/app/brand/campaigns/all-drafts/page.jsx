  ...
  return (
    <div className="bg-[#000201] min-h-full px-10 py-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{campaigns.map(c => <CampaignCard key={c._id || c.id} campaign={c} onClick={() => router.push('/brand/campaigns/' + (c._id || c.id))} />)}</div>
      ) : (
